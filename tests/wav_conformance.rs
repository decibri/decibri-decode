...
//! 3. channel count: 1, 2, more than 2, and more than eight
...
/// The cross product: every encoding, at four channel counts, at four lengths,
...
///
/// Nine is in the channel counts because WAV's `nChannels` is a `u16` and this
/// crate applies no ceiling below it. Eight is the largest count any format
/// this crate carries restricts itself to, so nine is the first value a
/// mistakenly global limit would refuse.
///
/// 8 * 4 * 4 * 7 * 3 * 2 = 5,376 files. Each is decoded through both paths and
...
fn the_dimension_matrix() {
    ...
    assert_eq!(files, 8 * 4 * 4 * 7 * 3 * 2);
}

// -- A nine-channel file from an outside encoder ------------------------------

/// A nine-channel WAV as ffmpeg 8.1.2 writes one: 22,050 Hz, 16-bit integer
/// PCM, twelve frames, and the `LIST`/`INFO` chunk ffmpeg puts between `fmt `
/// and `data`. Carried byte for byte rather than rebuilt, so the layout under
/// test is an encoder's and not this file's builder's.
///
...
const FFMPEG_NINE_CHANNEL_WAV: [u8; 294] = [
    ,
];

/// What ffmpeg 8.1.2 itself decodes that file to: the bytes of
/// `ffmpeg -i nine.wav -f f32le -c:a pcm_f32le`, 108 interleaved samples in
    ...
    for piece in [1usize, 7, 13, 64, 997, 65_536] {
        for drive in [Drive::Greedy, Drive::Interleaved] {
            ...
            assert_same_samples(
                &streamed,
                &expected,
                &format!("ffmpeg nine-channel WAV streamed at {piece} bytes, {drive:?}"),
            );
        }
    }
}
