//! Coverage-guided fuzzing of [`AiffStreamDecoder`], the AIFF and AIFF-C
//! reader for a stream that arrives in pieces.
//!
//! A streaming decoder has two input dimensions, the bytes and the boundaries
//! they arrive on, and both are under the mutator's control here. The 80-bit
//! extended-precision sample rate inside `COMM` is ten bytes that a push can
//! cut anywhere.
//!
...
use decibri_decode::AiffStreamDecoder;
